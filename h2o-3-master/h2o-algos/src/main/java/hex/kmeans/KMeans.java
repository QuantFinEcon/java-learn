 ...
 */
public class KMeans extends ClusteringModelBuilder<KMeansModel,
    ...
    public KMeans(boolean startup_once) {
        super(new KMeansModel.KMeansParameters(), startup_once);
    }
     ...
     */
    private static ClusterDist closest(double[][] centers, double[] point,
                                       String[][] isCats, ClusterDist cd, int count) {
        int min = -1;
    ...
    private static double[][] recluster(double[][] points, Random rand, int N,
                                        Initialization init, String[][] isCats) {
    ...
    private final class KMeansDriver extends Driver {
        ...
        double[][] initial_centers(KMeansModel model, final Vec[] vecs, final double[] means,
                                   final double[] mults, final int[] modes, int k) {
        ...
        public void computeImpl() {
            ...
            try {
                ...
                for (int k = startK; k <= _parms._k; ++k) {
                    ...
                    do { //Lloyds algorithm
                        assert (centers.length == k);
                        LloydsIterationTask task = new LloydsIterationTask(centers, means, mults, impute_cat, _isCats, k, hasWeightCol()).doAll(vecs2); //1 PASS OVER THE DATA
                        // Pick the max categorical level for cluster center
                        max_cats(task._cMeans, task._cats, _isCats);

                        // Handle the case where some centers go dry.  Rescue only 1 cluster
                        // per iteration ('cause we only tracked the 1 worst row)
                        if (!_parms._estimate_k && cleanupBadClusters(task, vecs, centers, means, mults, impute_cat))
                            continue;
